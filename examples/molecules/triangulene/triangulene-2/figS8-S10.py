...
for fn in ['dimer/', 'dimer-pentagon/']:
    ...
    ugrid, Esz0, Esz1, Esz2 = data[:,0], data[:,1], data[:,2], data[:,3]
    p = plot.Plot()
    if fn == 'dimer-pentagon/':
        # Compare to S_z=1 (ground state)
        p.axes.plot(ugrid, Esz0-Esz1, 'o', label='S$_{0}$-S$_{1}$')
        p.axes.plot(ugrid, Esz2-Esz1, 'o', label='S$_{2}$-S$_{1}$')
        p.set_ylabel(r'E$_{S_{i}}$-E$_{S_{1}}$ [eV]', fontsize=25)
    elif fn == 'dimer/':
        # Compare to S_z=2 (ground state)
        p.axes.plot(ugrid, Esz0-Esz2, 'o', label='S$_{0}$-S$_{2}$')
        p.axes.plot(ugrid, Esz1-Esz2, 'o', label='S$_{1}$-S$_{2}$')
        p.set_ylabel(r'E$_{S_{i}}$-E$_{S_{2}}$ [eV]', fontsize=25)
    ...
    p.axes.legend()
    p.savefig(fn+'energy-diff.pdf')

    H.polarize_sublattices()

    # Plot WF, spectrum and DOS of the GS 
    for u in [0., 3.0]:
        if u > 0:
            if fn == 'dimer-pentagon/':
                # GS in this case FM1

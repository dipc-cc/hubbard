...
def read(fn):
    ...
    mol = sisl.get_sile(fn).read_geometry()
    mol = mol.move(-mol.center(what='xyz'))
    mol.sc.set_nsc([1,1,1])

    # 3NN tight-binding model
    Hsp2 = sp2(mol, t1=2.7, t2=0.2, t3=0.18, dim=2)
...
def plot_spectrum(fn, H, mol, ncfile):
    ...
    p.axes.set_title(r'$E_{\uparrow}=%.2f$ eV'%(ev_up[H.Nup-1]), fontsize=40, y=-0.1)
    p.axes.axis('off')
    ...
    p.axes.set_title(r'$E_{\uparrow}=%.2f$ eV'%(ev_up[H.Nup]), fontsize=40, y=-0.1)
    p.axes.axis('off')
    ...
    p.axes.set_title(r'$E_{\downarrow}=%.2f$ eV'%(ev_dn[H.Ndn-1]), fontsize=40, y=-0.1)
    p.axes.axis('off')
